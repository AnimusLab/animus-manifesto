...
> * **Anchor Preventability:** High — Cryptographic Audit Integrity for HMAC-Chain-Protected Decision Records
...
I designed Anchor's Decision Audit Chain (DAC) to resolve this auditing gap. The DAC is partially active in the current release: HMAC-signed, hash-chained audit receipts are written to `.anchor/runtime_chain.jsonl` on every AI decision, providing tamper-evident chain integrity. The full forensic reconstruction CLI (`anchor verify --block`) and Hardware Security Module (HSM) signing integration are in active development. The case below describes the complete designed architecture — the cryptographic chain structure, the block data model, and the verification flow — as it will operate at full production deployment.
...
Anchor's Decision Audit Chain treats every execution state transition as a block in a localized, cryptographic ledger. Before any action is executed, a block is written containing the SHA-256 hashes of the system state, the active policy rule hash, the raw transaction payload, and a cryptographic signature.

**Current state:** The HMAC-signed, hash-chained JSONL ledger (`.anchor/runtime_chain.jsonl`) is active and writing on every AI decision. Chain integrity is verifiable via `DecisionAuditor.verify_chain()` in the Python API.

**In development:** The standalone `anchor verify --block` CLI command, HSM key integration, and the signed provenance certificate export are designed and specified in the current architecture but not yet shipped as standalone tools.
